...
var UserWhere = struct {
	...
	Token: whereHelperstring{field: "\"User\".\"token\""},
}
...
type userR struct {
	UserPermissions PermissionSlice `boil:"UserPermissions" json:"UserPermissions" toml:"UserPermissions" yaml:"UserPermissions"`
}
...
func (r *userR) GetUserPermissions() PermissionSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "salt", "token"}
	userColumnsWithoutDefault = []string{"id", "salt", "token"}
...
// UserPermissions retrieves all the Permission's Permissions with an executor via user_id column.
func (o *User) UserPermissions(mods ...qm.QueryMod) permissionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"Permission\".\"user_id\"=?", o.ID),
	)
...
func (userL) LoadUserPermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`Permission`),
		qm.WhereIn(`Permission.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddUserPermissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Permission) error {
	var err error
