...
func testPermissionsExists(t *testing.T) {
	...
	e, err := PermissionExists(ctx, tx, o.UserID, o.GameID)
	if err != nil {
...
func testPermissionsFind(t *testing.T) {
	...
	permissionFound, err := FindPermission(ctx, tx, o.UserID, o.GameID)
	if err != nil {
		t.Error(err)
	}
...
func testPermissionToOneGameUsingGame(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadGame(ctx, tx, false, (*[]*Permission)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Game == nil {
...
func testPermissionToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := PermissionSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Permission)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
		t.Error("struct should have been eager loaded")
...
func testPermissionToOneSetOpGameUsingGame(t *testing.T) {
	var err error
	...
	for i, x := range []*Game{&b, &c} {
		...
		if exists, err := PermissionExists(ctx, tx, a.UserID, a.GameID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
func testPermissionToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	permissionDBTypes = map[string]string{`UserID`: `VARCHAR(255)`, `GameID`: `TEXT`, `Permission`: `INT`}
	_                 = bytes.MinRead
)
