...
func testUserToManyUserPermissions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserPermissions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpUserPermissions(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `VARCHAR(255)`, `Salt`: `TEXT`, `Token`: `TEXT`}
	_           = bytes.MinRead
)
