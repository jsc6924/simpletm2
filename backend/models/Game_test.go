...
func testGameToManyGamePermissions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.GamePermissions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := GameSlice{&a}
	if err = a.L.LoadGamePermissions(ctx, tx, false, (*[]*Game)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.GamePermissions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.GamePermissions = nil
	if err = a.L.LoadGamePermissions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.GamePermissions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testGameToManyGameTranslates(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, translateDBTypes, false, translateColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, translateDBTypes, false, translateColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.GameTranslates().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := GameSlice{&a}
	if err = a.L.LoadGameTranslates(ctx, tx, false, (*[]*Game)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.GameTranslates); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.GameTranslates = nil
	if err = a.L.LoadGameTranslates(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.GameTranslates); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testGameToManyAddOpGamePermissions(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddGamePermissions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.GamePermissions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.GamePermissions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.GamePermissions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testGameToManyAddOpGameTranslates(t *testing.T) {
	var err error
	...
	var a Game
	var b, c, d, e Translate

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, gameDBTypes, false, strmangle.SetComplement(gamePrimaryKeyColumns, gameColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Translate{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, translateDBTypes, false, strmangle.SetComplement(translatePrimaryKeyColumns, translateColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Translate{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddGameTranslates(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.GameTranslates[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.GameTranslates[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.GameTranslates().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	gameDBTypes = map[string]string{`ID`: `TEXT`, `Title`: `TEXT`}
	_           = bytes.MinRead
)
